...
def takephoto():
    ...
    camera = PiCamera()
    camera.rotation = 180
    camera.capture('image%s.jpg'%contador)
...
def main():
    ...
    with open('image%s.jpg'%contador, 'rb') as image:
        ...
        try:
            ...
            print (texto)
            try:
                placa = re.findall(r'[P][0-9O][0-9O][0-9O][A-Z][A-Z][A-Z]',texto)[0]
                print (placa)
        ...
        global contador
        contador = contador + 1
        print("taking photo num: %s " % contador)
